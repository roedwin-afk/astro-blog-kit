...
// Fetchers para WordPress REST API.
// ─────────────────────────────────────────────────────────────
...
/**
 * Crea un cliente para la WordPress REST API.
 *
 * @example
 * const wp = createWPClient('https://mi-wordpress.com');
...
export function createWPClient(baseUrl: string) {
  ...
  /**
   * Obtiene lista de posts paginada.
   * Siempre incluye _embed para traer imagen y categorías.
   */
  async function getPosts(options: FetchPostsOptions = {}): Promise<{
    ...
    // Si hay filtro por categoría, primero obtenemos su ID
    if (categorySlug) {
    ...
    return {
      posts: data.map(normalizeWPPost),
      total,
      totalPages,
    };
  }
  ...
  /**
   * Obtiene todos los posts para getStaticPaths.
   * Hace múltiples requests si hay más de 100 posts.
   */
  ...
  /**
   * Obtiene todos los tags del blog.
   */
